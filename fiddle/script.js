...
const reusedText = {
  ...
  spanish: {
    ...
    intermediateKeyboardInstructions: 'Text("\\text{" + keyboardInstructions + "}", (0, 0), true, true)',
    displayedKeyboardInstructions: "Text(intermediateKeyboardInstructions, corner2 + keyInstructionsVec + (2xpixel, -2 ypixel), true, true)",
    keybindText: '"\\\\ \\\\Presiona k para " + (If(showKeyboardInstructions, "ocultar", "mostrar")) + " las instrucciones de teclado." + (If(true, "\\\\ \\\\Presiona x para reiniciar el ciclo de pestañas.", ""))',
    libSelectedObject: "",
    keyboardInstructionsConst: '{// A: "Press the arrow keys to move this point.", // example for a pointggbButton1: ggbObject.getValue("ggbButton1Enabled")? "Press space to ___.": unavailableButtonText,ggbButton2: ggbObject.getValue("ggbButton2Enabled")? "Press space to ___.": unavailableButtonText,ggbButton3: ggbObject.getValue("ggbButton3Enabled")? "Press space to ___.": unavailableButtonText,ggbButton4: ggbObject.getValue("ggbButton4Enabled")? "Press space to ___.": unavailableButtonText,ggbButton5: ggbObject.getValue("ggbButton5Enabled")? "Press space to ___.": unavailableButtonText,}',
  },
};
      ...
      switch (type) {
        ...
        case "list": {
          ...
          if (listXML.includes("comboBox")) {
            bigObject.english[el] = ggbApplet.getDefinitionString(el);
        ...
        default: {
          if (
            ...
            !Object.keys(reusedText.english).includes(el.concat("CaptionText"))
          ) {
    ...
    function pullReadText() {
      ...
      if (allMatches && allMatches.length !== 0) {
        allMatches.forEach(function(element, index) {
          bigObject.english.globalJSText["GGBReadText" + index] = element
            .replace(/([^\n][^\S\r\n])[^\S\r\n]+/g, "")
            .trim();
        });
      }
      ...
      if (
        ...
        allKeyboardInstructions[0].replace("const keyboardInstructions = ", "")
      ) {
        bigObject.english.keyboardInstructionsConst = allKeyboardInstructions[0]
      ...
      switch (type) {
        ...
        case "text": {
          if (ggbApplet.isIndependent(el)) {
            ...
            ggbApplet.evalCommand(el.concat("=", prettyAltText[language][el]));
          }
          break;
        }
        case "list": {
      ...
      if (altTextKeys.includes("globalJSText")) {
        ...
        ggbReadTextKeys.forEach((el, index) => {
          pulledGlobalJS = pulledGlobalJS.replace(
            prettyAltText.english.globalJSText["GGBReadText" + index],
            prettyAltText[language].globalJSText["GGBReadText" + index]
          );
        });
      }
    }
